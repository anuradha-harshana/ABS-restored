// hooks/Customer/customerReceipt.ts
...
const generateReceiptPdf = async (receiptData: any) => {
  ...
  try {
    ...
    const qrData = `${baseUrl}/receipt/${receiptData.id}`;
  ...
  doc.text(receiptData.customer_id?.toString() || "-", 160, 28);
  ...
  const customer = receiptData.customers;
  const customerInfo = [
    { label: "Customer Name", value: `${customer?.firstName || ""} ${customer?.lastName || ""}`.trim() || "-" },
    { label: "Phone Number", value: customer?.phoneNumber?.toString() || "-" },
    { label: "NIC Number", value: customer?.nic_no?.toString() || "-" },
    { label: "Address", value: customer?.address?.toString() || "-" },
  ];
  ...
  doc.text("We appreciate your business.", 20, y + 36);
